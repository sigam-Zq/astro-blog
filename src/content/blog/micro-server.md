...
kafka 作为多种类型的数据管道和消息系统来使用 , 由Linkedin 开发并开源的分布式消息系统,奇分布式及高吞吐率而被广泛使用. 基于发布订阅的消息系统
* 以时间复杂度为O(1)的方式提供消息持久化能力, TB级以上也能保证常数时间复杂度的访问性能
* 高吞吐率
* 支持 Kafka 间的消息分区, 及分布式消费,
* 离线处理,实时处理
* Scale out :在线水平扩展


为什么使用消息系统
* 解耦 在处理过程插入来一个隐含的基于数据的接口层,两边的处理过程都要实现这一接口, 允许独立扩展或者修改两边的处理过程, 确保他们准守相同的接口约束.
  而基于消息订阅发布的机制, 可以联动多个业务下流子系统,能不侵入的情况下分布编排和开发,来保证数据一致性 最终一致性
* 冗余 处理数据会失败, 除非数据持久化, 否则将会丢失, 消息队列把数据进行持久化,知道他们已经被完全处理, 通过这一方式规避了数据的丢失风险.
* 扩展性 因为这里解耦了处理过程, 所以增大消息入队和处理的频率是很简单的
* 灵活性 & 峰值处理能力 顶住突发的访问压力。(这里处理这种情况还有两个方案  1. autoScale 2. 服务降级)
* 可恢复性 一部分组件失效时,不会影响整个系统,后面恢复在消息队列中的消息也会恢复
* 顺序保证. 这里 可以保证一个partition 内的消息的有序性 (全局无序,局部有序)
* 缓冲 
* 异步通信

topic  

一个微服务用不同的topic 逻辑上一个topic 可以认为是一个queue
一个topic 可以有不同的schema
为了kafka 吞吐率可以线性提高, 可以把topic 可以分为多个partition


broker  一个Broker 是一个服务, 一个topic 可以有两个partition 在不同的Broker 上
从 Scale up 的角度考虑, 一个node 拥有越多的physical Disk 也可以负载更多的partition, 提升总IOPS

如果某topic 有N个Partition 集群有N+M 个Broker那么其中有N个Broker存储改topic 的一个partition, 剩下的M个Broker不存储该topic的partition数据. .....

topic 是逻辑概念, 真正在broker 分布式存储的是partition


kafka 消息存在于文件系统之上 这里,  这里使用了 bufferIO [direct-io](https://juejin.cn/post/7479296373331410984) 利用了 顺序IO 这里写的性能是好的, 这里Page Cache 达成的超高吞吐 . 这里任何发布到patition 的消息都被追加到partition 到尾部.这样顺序写磁盘操作让kafka 效率非常高

这里kafka的保存策略,支持配置的保留策略去删除旧数据(这里因为顺序的原因,这里不利于删除)


Offset 偏移量, 每条消息都有一个当前Patition 下唯一的64字节的Offset ,相当于当前分区第一条消息的偏移量,,即第几条消息,消费者可以指定消费的位置信息,当消费者挂掉再重新恢复的时候,可以从消费位置继续消费


这里的Partion是在一个broker 的主机上是一个目录的形式存在,每一个目录被平均分配成大小相等的Segment File 中, Seqment File 又由 index file 和 data file 组成, 他们总是成对出现, 后缀分别为 .index .log

> 00000000000000000000000000368769.index 

1,0  -----> Message368770   0

3,497  -----> Message368772. 497 
...
这里 的368769就是索引
这里 是[稀疏索引](https://blog.csdn.net/weixin_43114209/article/details/144444685) 减少索引文件大小,使得能把index 映射到内存,降低了查询时的磁盘IO, 同时也没有给查询带来很多的时间消耗
这里的文件名都是 上一个Segment 最后一条消息的offset, 需要查找,这里只需要根据文件名进行二分查找就可以很快找到

这里Partition 在每一条 Message都包括以下三个属性
* Offset : 表示Message 在当前Partition 中的偏移量,一个逻辑上的值,可以唯一确定一个patition 中的一条message
* MessageSize 一个消息的Data 大小
* Data Message 的内容


0.10.0.0 版本起, 为分片日志文件中新增了一个 timeindex的索引文件, 可以根据时间戳定位消息


### 12.2 Producer 和 Consumer


producer 


* 指定 partition 直接将给定的value 作为partition的值
* 没有指定 但是有key的情况下, key的hash值然后和分区数取余确认分区
* 上面都没指定,给一个整数(后面在这个整数上做自增)作为key 会使用Round-robin 轮询算法, 使用整数取余后确认存储到哪一个partition

这里总体是做负载均衡
发送到partition 这里收到后返回ACK指令, 然后进行下一轮的发送,不然会重新发送数据
0.11 版本 实现了, 重复发送消息的接收方做了幂等性(这里不完善 使用ticket原理,重启会丢失,且跨partition 失效 不完善) 没法彻底解决[ Exactiy once](https://zhuanlan.zhihu.com/p/431414055)


cousumer

这里有消费组的概念, G1 一个消费组能存在多个消费组一个, 一个topic 对应一个消费组, 这里最好,一个partirion 对应一个消费者, 消费者多余partition, 会有消费者没有消息


这里也可以 写入一次消息, 但是可以后面两个消费组进行消费, 换言之, 每个应用都可以得到全量的消息
但是 这里如果两个消费组消费能力速度不一样的情况下,会导致kafka的性能衰减(如果两个 消费组会导致 page cache污染)


之前使用Zookeeper 实现均分流量, 后面是选出来一个broker来做组协调者 接受心跳信息

当一个消费者正常心跳但是很长时间没有消费的时候这个时候的[活锁](https://www.zhihu.com/question/20566246) 会触发一个 rebalance 操作,重新负载均衡 ,但是这里操作会导致吞吐能力下降


Consumer commit Offset

这里消费端,可以通过设置参数enable.auto.commit 来控制是手动提交还是自动提交, 自动提交默认为5s 由auto.commit.interval.ms 控制


这里如果消费过程遇见rebalance ,这里就会从上一次的消费提交位置重新消费, 这里会存在重复消费


这里为了解决 Exacly Once的问题 可以使用 Flink的 二阶段提交解决这个问题


这里的 Producer 是push 模式 Consumer 是pull 模式

这里push 很难适应消费速率不同的消费者, 因为消息发送速率是由Broker 决定的,这里更适合pull模式,因为这样能简化设计
这里可以让消费者自行commit offset 和自主实现 at most once 和 at least once

[At most once, at least once, exactly once](https://blog.bytebytego.com/p/at-most-once-at-least-once-exactly)


pull 缺点是 没有消息消费的时候会空转轮询, 但是这里可以通过长轮询来优化
> [长轮询等方式](https://www.cnblogs.com/qiqi715/p/13138589.html)



### 12.3 Leader 和 Follower

Kafka0.8 之前 没有HA机制

这和之前设计等数据持久性和 Delivery 和 Guarantee等设计目标相悖

producer 分为同步模式和异步模式 同步可以读取配置等message.send.max.retries (默认为3) 次后抛出异常, 用户可以选择停止发送或者继续发送后续,前者会阻塞,而后者会造成消息丢失
异步也会发送配置等次数后继续发送,这时候只能通过日志发现数据等丢失
