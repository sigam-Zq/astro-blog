...
> Google File System (GFS) 是一个为处理海量数据而设计的专有分布式文件系统。它的基本运作原理可以概括为：将大文件切分成固定大小的数据块（Chunk），通过一个中心化的主服务器（Master）管理元数据，并将数据块在多个数据块服务器（Chunkserver）上复制存储，以此实现高吞吐量、容错性和可扩展性


🏗️ 核心架构：三大组件
一个GFS集群由三个主要部分组成，它们协同工作：

客户端 (Client)：这是GFS提供给应用程序的访问接口，以库文件的形式存在。应用程序通过调用这些库函数，与Master和Chunkserver通信，完成数据的读写操作，整个过程对上层应用透明。

主服务器 (Master)：这是GFS的核心管理节点，通常只有一个。它不存储实际数据，而是管理所有元数据 (Metadata)。元数据包括：

命名空间 (Namespace)：即文件和目录的树状结构。

文件到Chunk的映射：记录每个文件由哪些Chunk组成。

Chunk副本的位置：记录每个Chunk的所有副本分别存储在哪些Chunkserver上。

为了追求速度，所有这些元数据都保存在Master的内存中。同时，为了持久化和恢复，命名空间和文件映射关系会以操作日志（Operation Log） 的形式记录在磁盘上，并在远程机器上备份。

数据块服务器 (Chunkserver)：这是实际存储数据的节点，集群中通常有大量Chunkserver。它们负责在本地磁盘上以普通Linux文件的形式存储Chunk，并响应客户端的读写请求


split brain (脑裂问题) 出现两个主 且不知道彼此的存在
